...
int lfPCQentry_pingtest(SPHMultiPCQueue_t pqueue, SPHMultiPCQueue_t cqueue,
	...
	handle = SPHMultiPCQueueAllocStrideDirectSpinTM(pqueue);
	if (handle) {
		...
		handle = SPHMultiPCQueueGetNextCompleteDirectSpinTM(cqueue);
		if (handle) {
			array = (int *) SPHLFEntryDirectGetFreePtr(handle);
			rc1 = (array[0] != val1);
			rc2 = (array[1] != val2);
...
lfPCQentry_pongtest(SPHMultiPCQueue_t pqueue, SPHMultiPCQueue_t cqueue,
	...
	handle = SPHMultiPCQueueGetNextCompleteDirectSpinTM(pqueue);
	if (handle) {
		array = (int *) SPHLFEntryDirectGetFreePtr(handle);
		rc1 = (array[0] != val1);
		rc2 = (array[1] != val2);
		...
		handle2 = SPHMultiPCQueueAllocStrideDirectSpinTM(cqueue);
		if (handle2) {
...
lfPCQentry_pongpause(SPHMultiPCQueue_t pqueue, SPHMultiPCQueue_t cqueue,
	...
	handle = SPHMultiPCQueueGetNextCompleteDirectSpinPauseTM(pqueue);
	if (handle) {
		array = (int *) SPHLFEntryDirectGetFreePtr(handle);
		rc1 = (array[0] != val1);
		rc2 = (array[1] != val2);
		...
		handle2 = SPHMultiPCQueueAllocStrideDirectSpinPauseTM(cqueue);
		if (handle2) {
			array2 = (int *) SPHLFEntryDirectGetFreePtr(handle2);
			array2[0] = array[0];
			array2[1] = array[1];
...
int test_thread_Producer_pinger(SPHMultiPCQueue_t pqueue,
	...
	for (i = 0; i < iterations; i++) {
		...
		if (!rc) {
			...
			printf(
					"test_thread_Producer_pinger SPHMultiPCQueueAllocDirectSpinTM (%p) failed\n",
					pcqueue);
			rtn++;
			break;
		}
...
int test_thread_Producer_pingpause(SPHMultiPCQueue_t pqueue,
	...
	for (i = 0; i < iterations; i++) {
		...
		if (!rc) {
			...
			printf(
					"test_thread_Producer_pingpause SPHMultiPCQueueAllocDirectSpinTM (%p) failed\n",
					pcqueue);
			rtn++;
			break;
		}
			       ...
			       int thread_ID, long iterations)
{
  ...
  for (i = 0; i < iterations; i++)
	...
	{
	  printf
	    ("test_thread_Producer_fasterfill SPHMultiPCQueueDirectSpin (%p) failed\n",
	     pcqueue);
	  rtn++;
	  break;
	}
